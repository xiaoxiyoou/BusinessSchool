<template>
  <div class="container">
    ...
    <input v-model="codeVaue" class="phone" type="tel" placeholder="请输入赠送码">
    <div class="confirm" @click="_submitPersonInfo">
      提交
    </div>
    <!-- <div class="confirm" @click="Extion()">
      进入首页
    </div> -->
...
export default {
  ...
  mounted() {
    let url = decodeURIComponent(window.location.hash).split("#/giveInformation?")[1].split("|")
    console.log('url', url)
    this.FreeCode = url[1].split("freeCode=")[1]
    this.shareUserId = url[0].split("shareUserId=")[1]
    console.log('url', url[0].split("shareUserId=")[1])
    console.log('url', url[1].split("freeCode=")[1])
